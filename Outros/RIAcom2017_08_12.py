import serial, time, sys, os, threading, struct
import numpy as np
...
##        var.rack4 = var.ria.ui.check4.isChecked()
    ...
    def v_converter(self, resp, i):
        ...
        for j in range(16):
            ...
            self.tmp0 = resp[(8*j+11) : (8*j+12)]
            self.tmp1 = resp[(8*j+9) : (8*j+10)]
            self.tmp2 = resp[(8*j+7) : (8*j+8)]
            self.tmp3 = resp[(8*j+5) : (8*j+6)]
            self.vtmp = np.append(self.vtmp, (self.tmp0 + self.tmp1 + self.tmp2 +
                                                     self.tmp3))
            
            self.output = struct.unpack('f', self.vtmp[j])
#===============================================================================
...
#===============================================================================
            ...
            if divmod(j, 2)[1] == 0:
                ...
                self.T = np.append(self.T, self.output*5)
