...
from Fogale_UI_vCiencAberta import Ui_MainWindow #novo
...
from python_pyqt_NewVersion_vCiencAberta import plot
...
##        self.ser = serial.Serial('COM7', 115200, 8, 'N', timeout = .2, stopbits = 1)
    ...
    def ui_init(self):
        ...
        """CIENCIA ABERTA"""
        self.numSensor = 7
        QtCore.QObject.connect(var.ria.ui.btnTexto, QtCore.SIGNAL("clicked()"), self.setaReferencia)
        self.thread_text = Thread_text(self)
        self.connect(self.thread_text, QtCore.SIGNAL('setarTexto'), self.escreveTexto)
        self.text = ""
        self.indexBoxCienc = 2
        self.valRef = 0
        self.referencia = "sensor2"
        var.ria.ui.refBoxCienc.addItems(["Absoluta","Sensor próprio","Sensor adjacente"])
        QtCore.QObject.connect(var.ria.ui.refBoxCienc, QtCore.SIGNAL("currentIndexChanged(int)"),
                                                     self.mudaReferencia)
        var.ria.ui.widget_cienc.Data[0] = dataclass()
        self.rangeX = 2

        self.thread_imagem = Thread_imagem(self)
        self.connect(self.thread_imagem, QtCore.SIGNAL('imagem'), self.printImagem)
        self.numImagem = 1
        
        QtCore.QObject.connect(var.ria.ui.numPontos_cienc, QtCore.SIGNAL("valueChanged (int)"),
                                                     self.set_numPontos_cienc)
        var.ria.ui.numPontos_cienc.setValue(self.rangeX)

        QtCore.QObject.connect(var.ria.ui.imageOnOff, QtCore.SIGNAL("stateChanged(int)"),
                                                     self.set_imageOnOff)

        self.imagemHabilitada = False
        self.j = 0 #contador da leituras
        self.somaVal = 0
        ...
        QtCore.QObject.connect(var.ria.ui.PlotBox8, QtCore.SIGNAL("currentIndexChanged(int)"),
                                                     self.set_plot8)
        QtCore.QObject.connect(var.ria.ui.t_aq, QtCore.SIGNAL("valueChanged (float)"),
                                                     self.set_t_aq)
    ...
    def refresh_text_box(self,MYSTRING): 
        var.ria.ui.logOutput_cienc.append('started appending %s' % MYSTRING) #append string
...
##                self.output = self.output*5
    ...
    def acquire(self, address):
        for i in address:
            if i < 5 and i > 0:
                ...
                if self.response == "":
                    ...
                    if i == 1:
                        ...
                        try:
                            ...
                            """adequação para ciencia aberta"""
                            #self.valAux = var.D1[-1][0]
                            #self.text = str(self.valAux[9])
                            #self.chamaTexto()
                        except ValueError:
                ...
                try:
                    self.vetorValD = var.D1[-1][1:]
                    #self.valD1 = self.vetorValD[self.numSensor]
                    #self.valD2 = self.vetorValD[self.numSensor-1]
                    self.valMostrado = self.vetorValD[self.numSensor]-self.valRef

                    if(self.valMostrado < 0):
                        var.ria.ui.txtLogNeg.setText("-")
                    else:
                        var.ria.ui.txtLogNeg.setText("")
                    
                    """inicio teste com média"""
                    if(self.j < 2):#funcionam: 5, t=0.05; 5,t=0.02; 2, t = 0.05
                        self.somaVal += self.valMostrado
                        if(self.j == 0):
                            self.valPlot = self.valMostrado
                        self.j += 1
                    else:
                        self.valPlot = self.somaVal/2
                        self.j = 0
                        self.somaVal = 0
                    self.text = str("{:.3f}".format(abs(round(self.valPlot,3)))) #formato 0.010
                    #self.text = str(round(self.valMostrado,3))
                    self.chamaTexto()
                    self.plot_cienc(round(self.valPlot,4))
                    
                    
                    #self.text = str("{:.3f}".format(round(self.valMostrado,3))) #formato 0.010
                    #self.text = str(round(self.valMostrado,3))
                    #self.chamaTexto()
                    #self.plot_cienc(round(self.valMostrado,4))
                    if(self.imagemHabilitada):
                        self.chamaImagem()
                    else:
                        self.numImagem = 0
                        self.chamaImagem()
    ...
    def plot_cienc(self, plotData):
        self.date = time.strftime("%Y_%m_%d", time.localtime()) #adquire data
        self.a = time.localtime()
        self.str_hora = time.mktime(self.a)

        """atribuicao de dados as variaveis do plot"""
        var.ria.ui.widget_cienc.Data[0].y = np.append(
                                    var.ria.ui.widget_cienc.Data[0].y, plotData)
        var.ria.ui.widget_cienc.Data[0].x = np.append(
                                    var.ria.ui.widget_cienc.Data[0].x, self.str_hora)
        ...
        """LIMITA RANGE DO EIXO X"""
        if len(var.ria.ui.widget_cienc.Data[0].x) > self.rangeX:
            self.dif = len(var.ria.ui.widget_cienc.Data[0].x) - self.rangeX
            var.ria.ui.widget_cienc.Data[0].x = var.ria.ui.widget_cienc.Data[0].x[self.dif:]
            var.ria.ui.widget_cienc.Data[0].y = var.ria.ui.widget_cienc.Data[0].y[self.dif:]
...
#            plot.set_dataList()
    ...
    def chamaTexto(self):
        self.thread_text.quit()
        self.thread_text.start()

    def escreveTexto(self):
        var.ria.ui.logOutput_cienc.setPlainText(self.text)
        #var.ria.ui.txtLogNeg.setText("-")
        #var.ria.ui.logOutput_cienc.insertPlainText(self.valorSensCienc)

    def mudaReferencia(self):
        self.indexBoxCienc = var.ria.ui.refBoxCienc.currentIndex()
        if self.indexBoxCienc == 0:
            self.refencia = "abs"
        elif self.indexBoxCienc == 1:
            self.referencia = "sensor1"
        elif self.indexBoxCienc == 2:
            self.referencia = "sensor2"

    def setaReferencia(self):
        if self.referencia == "abs":
            self.valRef = 0;
        if self.referencia == "sensor1":
            self.valRef = self.vetorValD[self.numSensor]   
        if self.referencia == "sensor2":
            self.valRef = self.vetorValD[self.numSensor-1] 

    def chamaImagem(self):
        self.thread_imagem.quit()
        self.thread_imagem.start()
    
    def printImagem(self):
        if(var.ria.ui.imageOnOff.isChecked()):
            #if(abs(round(self.valMostrado,3)) <= 0.010):
            if(abs(round(self.valPlot,3)) <= 0.010):
                self.numImagem = 1 #BACTECTIA
            #elif(abs(round(self.valMostrado,3)) >= 0.025 and abs(round(self.valMostrado,3)) <= 0.035 ):
            elif(abs(round(self.valPlot,3)) >= 0.025 and abs(round(self.valPlot,3)) <= 0.035 ):
                self.numImagem = 2 #TOLERANCIA ALINHAM.
            #elif(abs(round(self.valMostrado,3)) >= 0.060 and abs(round(self.valMostrado,3)) <= 0.080 ):
            elif(abs(round(self.valPlot,3)) >= 0.060 and abs(round(self.valPlot,3)) <= 0.080 ):
                self.numImagem = 3 #FIO DE CABELO
            else:
                self.numImagem = 0
            #self.chamaImagem()
        #else:
        #    self.numImagem = 0
        if self.numImagem == 0:
            var.ria.ui.imagem1.clear()
            var.ria.ui.txtImag.setText("")
        if self.numImagem == 1: # BACTERIA
            #var.ria.ui.groupImagem.resize(var.ria.ui.pixmap1.width()+20,var.ria.ui.pixmap1.height()+20)
            #var.ria.ui.imagem1.resize(var.ria.ui.pixmap1.width(),var.ria.ui.pixmap1.height())
            var.ria.ui.imagem1.setGeometry(10, 10, var.ria.ui.pixmap1.width(),var.ria.ui.pixmap1.height())
            var.ria.ui.imagem1.setPixmap(var.ria.ui.pixmap1)
            var.ria.ui.txtImag.setText("Bactéria: 0,001 - 0,005 mm")
        if self.numImagem == 2: # MAGNETO
            #var.ria.ui.groupImagem.resize(var.ria.ui.pixmap2.width()+20,var.ria.ui.pixmap2.height()+20)
            var.ria.ui.imagem1.setGeometry(30, 10, var.ria.ui.pixmap2.width(),var.ria.ui.pixmap2.height())
            #var.ria.ui.imagem1.resize(var.ria.ui.pixmap2.width(),var.ria.ui.pixmap2.height())
            var.ria.ui.imagem1.setPixmap(var.ria.ui.pixmap2)
            var.ria.ui.txtImag.setText("Tolerância de alinhamento dos\n magnetos do Sirius: 0,040 mm")
        if self.numImagem == 3: # FIO DE CABELO
            #var.ria.ui.groupImagem.resize(var.ria.ui.pixmap3.width()+20,var.ria.ui.pixmap3.height()+20)
            #var.ria.ui.imagem1.resize(var.ria.ui.pixmap3.width(),var.ria.ui.pixmap3.height())
            var.ria.ui.imagem1.setGeometry(10, 40, var.ria.ui.pixmap3.width(),var.ria.ui.pixmap3.height())
            var.ria.ui.imagem1.setPixmap(var.ria.ui.pixmap3)
            var.ria.ui.txtImag.setText("Espessura de um fio de cabelo:\n         ~0,070 mm")
            
    def set_numPontos_cienc(self):
        self.rangeX = var.ria.ui.numPontos_cienc.value()

    def set_imageOnOff(self):
        if(var.ria.ui.imageOnOff.isChecked()):
            self.imagemHabilitada = True
        else:
            self.imagemHabilitada = False
...
class Thread_text(QtCore.QThread):
    def __init__(self,parent):
        QtCore.QThread.__init__(self, parent)

    def run (self):
        self.emit(QtCore.SIGNAL('setarTexto'))

class Thread_imagem(QtCore.QThread):
    def __init__(self,parent):
        QtCore.QThread.__init__(self, parent)

    def run (self):
        self.emit(QtCore.SIGNAL('imagem'))
