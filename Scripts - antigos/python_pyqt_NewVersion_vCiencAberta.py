...
class PlotOnOff:
   ...
   #seta valores de referencia com botao 'Set Ref' 
                ...
                if(aux.plotSet == "nivel"): #PLOT DE NIVEL
                    try:
                        var.ria.ui.widget_on.Data[(i*8)+j].y = np.append(
                                    var.ria.ui.widget_on.Data[(i*8)+j].y,
                                    self.D[j+1])
                        #var.ria.ui.logOutput_cienc.insertPlainText(str(self.D[j+1]))
                        #self.escreve(str(self.D[j+1]))
                    except:
                        print("Erro 12")
                        raise
                else: #PLOT DE TEMPERATURA
    ...
    """def escreve(self,MYSTRING): 
        var.ria.ui.logOutput_cienc.insertPlainText(MYSTRING+'\n') #append string
        var.ria.ui.logOutput_cienc.moveCursor(QTextCursor.End)
